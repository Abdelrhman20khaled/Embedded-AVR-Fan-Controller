...
uint16 ADC_readChannel(uint8 channel_num){
	// take the channel number as a 3 bit and do bitwise OR with ADMUX register.
	ADMUX = (ADMUX & 0xE0) | (channel_num & 0x07);
	// put this pin by one to tell the ADC start conversion.
	SET_BIT(ADCSRA,ADSC);
	// wait until the ADC finishes it is process.
	while(BIT_IS_CLEAR(ADCSRA,ADIF));
	//clear flag by put it by 1.
	SET_BIT(ADCSRA,ADIF);
	//return the value stored in ADC register
	return ADC;
}
